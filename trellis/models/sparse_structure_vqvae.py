...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z,only_return_indices=False):
        ...
        if only_return_indices:
            # Chunked argmin avoids a full [N, V] distance matrix (OOM on long eval runs).
            codebook = self.embeddings.weight
            codebook_norm = (codebook * codebook).sum(dim=1)
            n = z_flatten.shape[0]
            encoding_indices = torch.empty(n, dtype=torch.long, device=z_flatten.device)
            chunk = 256
            for start in range(0, n, chunk):
                end = min(start + chunk, n)
                zc = z_flatten[start:end]
                z_norm = (zc * zc).sum(dim=1, keepdim=True)
                dist = z_norm + codebook_norm.unsqueeze(0) - 2.0 * (zc @ codebook.t())
                encoding_indices[start:end] = dist.argmin(dim=1)
